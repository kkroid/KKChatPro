...
public class GlobalHttpHandlerImpl implements GlobalHttpHandler {
    ...
    public Response onHttpResultResponse(@Nullable String httpResult,
        ...
        //     try {
        //         List<User> list = ArmsUtils.obtainAppComponentFromContext(mContext).gson().fromJson(httpResult, new TypeToken<List<User>>() {
        //         }.getType());
    ...
    public Request onHttpRequestBefore(@NonNull Interceptor.Chain chain, @NonNull Request request) {
        ...
        if (mCookie == null) {
            ...
            Timber.d("use cookies:%s", mCookie);
            // 查看newBuilder方法可以知道，newBuilder会将现有参数设置到新的request中，
            // 所以这里新建的request还保留之前的参数
            return chain.request().newBuilder().header("Cookie", mCookie)
                    .build();
        }
    }
}
