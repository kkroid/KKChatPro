...
//    public static String sDomain = Api.WAN_ANDROID_DOMAIN;
    ...
    public void applyOptions(@NonNull Context context, @NonNull GlobalConfigModule.Builder builder) {
        ...
        builder.baseurl(Api.WAN_ANDROID_DOMAIN)
                ...
                // 若需要无限次的切换 BaseUrl, 以及各种复杂的应用场景还是需要使用 RetrofitUrlManager 框架
                // 以下代码只是配置, 还要使用 Okhttp (AppComponent 中提供) 请求服务器获取到正确的 BaseUrl
                // 后赋值给 GlobalConfiguration.sDomain
...
//                .executorService(Executors.newCachedThreadPool())
                ...
                // 可以做一些操作, 比如 Token 超时后, 重新获取 Token
                ...
                .gsonConfiguration((ctx, gsonBuilder) -> gsonBuilder
                        .setPrettyPrinting()
                        // 支持序列化值为 null 的参数
                        .serializeNulls()
...
//                    okhttpBuilder.sslSocketFactory(); // 支持 Https, 详情请百度
                    ...
                    // 详细使用方法请查看 https://github.com/JessYanCoding/ProgressManager
                    // ProgressManager.getInstance().with(okhttpBuilder);
                    // 让 Retrofit 同时支持多个 BaseUrl 以及动态改变 BaseUrl, 详细使用方法请查看
                    // https://github.com/JessYanCoding/RetrofitUrlManager
                    RetrofitUrlManager.getInstance().with(okhttpBuilder);
                    // RetrofitUrlManager.getInstance().putDomain(Api.DOMAIN_NAME_WAN_ANDROID, Api.WAN_ANDROID_DOMAIN);
                    // RetrofitUrlManager.getInstance().putDomain(Api.DOMAIN_NAME_MNZP, Api.MXNZP_DOMAIN);
                })
                // 这里可以自己自定义配置 RxCache 的参数
                .rxCacheConfiguration((ctx, rxCacheBuilder) -> {
